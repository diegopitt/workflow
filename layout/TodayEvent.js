...
const TodayEvent = (props) => {
  ...
  const [passed, setPassed] = useState(moment().isAfter(moment(event.end, 'HH:mm')) ? true : false)
  const [isNow, setIsNow] = useState(moment().isAfter(moment(event.start, 'HH:mm')) && moment().isBefore(moment(event.end, 'HH:mm')) ? true : false)
  const [timeLeft, setTimeLeft] = useState(moment().locale('es').to(moment(event.start, 'HH:mm')))
  const dayName = () => {
    ...
    event && <div className="flex">
			<div className="flex block px-4 py-2 sm:px-4 sm:py-2 md:px-6 md:py-2 text-sm">
				<div className="flex-shrink-0 h-14 w-14 sm:h-14 sm:w-14 md:h-18 md:w-18">
					<img className="h-14 w-14 sm:h-14 sm:w-14 md:h-18 md:w-18 rounded-xl w-full h-full object-cover" src={event.img} alt="" />
				</div>
