...
const UserProvider = ({ children }) => {
  ...
  const registerEvent = async (uid, eventKey, start, title, Phone) => {
    let updates = {};
    const a = await firebase.app().database().ref(`/events/${eventKey}/dayofweek`).once('value').then((days) => {
      updates[`/eventsByUser/${uid}/${eventKey}`] = true;
      updates[`/usersByEvent/${eventKey}/${uid}`] = true;
      Object.keys(days.val()).forEach(day => {
        updates[`/eventsByUserByDay/${day}/${uid}/${eventKey}`] = true
        updates[`/eventsNotification/${day}/${uid}`] = { phone: Phone, start: start, title: title };
      })
      return firebase.app().database().ref().update(updates)
    })
  }
  const saveFavorite = async (uid, eventKey) => {
    return firebase.app().database().ref(`/favotitesByUser/${uid}/${eventKey}`).set(true)
  }
